...
void Room::splitPolygon(Side currentSide, polygon4uv* polyPtr, polyNode p, polyNode q)
{
	...
	if (polyPtr->leftChild == NULL && polyPtr->rightChild == NULL)	//If polygon is childless
	{
		...
		if (checkPolygonForSplitting(*polyPtr, p, q))
		{
			...
			polyPtr->leftChild->leftChild = NULL;
			polyPtr->leftChild->rightChild = NULL;
			printf("Polygon is split into leftChild...");

			polyPtr->rightChild = new polygon4uv;
			polyPtr->rightChild->leftChild = NULL;
			polyPtr->rightChild->rightChild = NULL;
			printf("...and rightChild.\n");

			//polyPtr->rightChild->counter += 1;
	...
	else if(polyPtr->leftChild != NULL && polyPtr->rightChild != NULL)
	{
...
{
	if (polyTree->leftChild == NULL && polyTree->rightChild == NULL)
	{
		printf("\nPOLYGON IS SAVED...\n\n");
		triangle3dV T;
	...
	else if (polyTree->leftChild != NULL && polyTree->rightChild != NULL)
	{
...
void Room::calculateMesh()
{
	...
	if (isBottomOn)
	{
		...
		if (openingOnBottom)
		{
			...
			currentPoly.leftChild	= NULL;
			currentPoly.rightChild	= NULL;
			//currentPoly.counter		= 0;

			if (openingBottom != NULL)
			{
...
triangle3dV* Room::getTriangleData()
{
	...
	triangle3dV* t = new triangle3dV[tCount];
	...
	for (int i = 0; i < bottomPolyContainer.size(); i++, tC++)
	{
		if (tC < tCount)
		{
			...
			t[tC].A.x += posX;	t[tC].A.y += posY;	t[tC].A.z += posZ;
			t[tC].B.x += posX;	t[tC].B.y += posY;	t[tC].B.z += posZ;
			t[tC].C.x += posX;	t[tC].C.y += posY;	t[tC].C.z += posZ;
		}
	}
