...
Uint32 fillTriangle(coord2* t, Uint32* pixels, int width, int height, Uint32 colour)
{
	
	int yMin, yMax, yTemp;

	yTemp = (t[0].y < t[1].y) ? t[0].y : t[1].y;
	yMin = (t[2].y < yTemp) ? t[2].y : yTemp;

	yTemp = (t[0].y > t[1].y) ? t[0].y : t[1].y;
	yMax = (t[2].y > yTemp) ? t[2].y : yTemp;
	...
	for (int hg = yMin; hg < yMax; hg++)
	{
		endIndex = 0;
		//Side 0-1:
		if ((t[0].y <= hg && t[1].y > hg) || (t[1].y <= hg && t[0].y > hg))
		{
			dx = t[1].x - t[0].x; dy = t[1].y - t[0].y;
			yy = (double)hg - (double)t[0].y; xx = dx * (yy / dy);
			wd = t[0].x + int(xx);
			if (endIndex < 2)
			{
				lineEnd[endIndex] = wd;
		...
		//Side 1-2:
		if ((t[1].y <= hg && t[2].y > hg) || (t[2].y <= hg && t[1].y > hg))
		{
			dx = t[2].x - t[1].x; dy = t[2].y - t[1].y;
			yy = (double)hg - (double)t[1].y; xx = dx * (yy / dy);
			wd = t[1].x + int(xx);
			if (endIndex < 2)
			{
				lineEnd[endIndex] = wd;
		...
		//Side 2-0:
		if ((t[2].y <= hg && t[0].y > hg) || (t[0].y <= hg && t[2].y > hg))
		{
			dx = t[0].x - t[2].x; dy = t[0].y - t[2].y;
			yy = (double)hg - (double)t[2].y; xx = dx * (yy / dy);
			wd = t[2].x + int(xx);
			if (endIndex < 2)
			{
				lineEnd[endIndex] = wd;
