...
coord2 Camera::view2screen(vect3 vertex, double hR, double vR)
{
	...
	//pixel.x = roundInt((w * 0.5 + (vertex.x / vertex.z) * w * 0.5 * hR * 0.95));
	//pixel.y = roundInt((h * 0.5 - (vertex.y / vertex.z) * w * 0.5 * vR * 0.95));
	pixel.z = 1 / vertex.z;
...
void Camera::fillTriangle2(triangle2dG t, Canvas screen, bool d)
{
			...
			for (int i = startX; i < endX + 1; i++)
			{
				if ((i >= 0 && i < SCREEN_WIDTH) && (hg >= 0 && hg < SCREEN_HEIGHT))
				{
					...
					if (1 / zCurrent < screen.depthBuffer[hg * w + i])
					{
						...
						{
							screen.pixelBuffer[2 * hg * SCREEN_WIDTH + 2 * i] = getColour(0, (byte)(r * illCurrent), (byte)(g * illCurrent), (byte)(b * illCurrent));

							screen.pixelBuffer[2 * hg * SCREEN_WIDTH + 2 * i + 1] = getColour(0, (byte)(r * illCurrent), (byte)(g * illCurrent), (byte)(b * illCurrent));

							screen.pixelBuffer[(2 * hg + 1) * SCREEN_WIDTH + 2 * i] = getColour(0, (byte)(r * illCurrent), (byte)(g * illCurrent), (byte)(b * illCurrent));

							screen.pixelBuffer[(2 * hg + 1) * SCREEN_WIDTH + 2 * i + 1] = getColour(0, (byte)(r * illCurrent), (byte)(g * illCurrent), (byte)(b * illCurrent));
						}
						screen.depthBuffer[hg * w + i] = 1 / zCurrent;
					}
				}
