...
void Camera::projectPoly(int n, vect3* vertexList, Uint32 colour, Canvas screen, double hRatio, double vRatio)
{
	triangle2dG screenT;
	...
	for (int i = 0; i < n - 2; i++)
	{
		...
		this->fillTriangle(screenT, screen);

	}
}


void Camera::fillTriangle(triangle2dG t, Canvas screen)
{
			...
			for (int i = startX; i < endX + 1; i++)
			{
				if ((i >= 0 && i < w) && (hg >= 0 && hg < h))
				{
					...
					if (1 / zCurrent < screen.depthBuffer[hg * w + i])
					{
						...
						/*pixels[hg * w + i] = getColour(0, (byte)(r * illCurrent / zCurrent * 10),
								(byte)(g * illCurrent / zCurrent * 10), (byte)(b * illCurrent / zCurrent * 10));*/
						//pixels[hg * w + i] = getColour(0, (byte)(255 / zCurrent), (byte)(255 / zCurrent), (byte)(255 / zCurrent));
						//screen.pixelBuffer[hg * w + i] = 255;
...
void Camera::fillTriangleZ(triangle2dG t, Canvas screen)
{
	coord2 pt[3] = { t.a, t.b, t.c };
	int yMin, yMax;
