...
public:
	...
	void illuminatePoly(LightSource*, triangle3dV*, bool);
	coord2 view2screen(vect3 vertex, double hR, double vR);
	vect3 screen2view(coord2 pixel, double hR, double vR);
	void view2text();
	void projectPoly(int, vect3*, textCoord*, Uint32, Canvas, double, double, projectionStyle, int, vect3*, double*, triangle3dV);
	...
	void fillTriangleTTorch(triangle3dV, triangle2dG, Canvas, bool, double);
	...
	void drawTriangle(triangle2dG, Canvas);
	void drawGradientTriangle(triangle2dG, Canvas);
	void drawLine(coord2, coord2, Uint32, Canvas);
	void drawGradientLine(coord2, coord2, Uint32, Canvas);
