...
void Camera::illuminatePoly(LightSource* L, triangle3dV* T, bool G)
{
	int nLight = sizeof(*L) / sizeof(LightSource);
	double illumAll, illumA, illumB, illumC;
	illumAll = illumA = illumB = illumC = 0.0;
...
void Camera::projectPoly(int n, vect3* vertexList, textCoord* uvList, Uint32 colour, Canvas screen,
		double hRatio, double vRatio, projectionStyle style, int nLamp, vect3* lampPos, double* lampInt, triangle3dV originalPoly)
{
	triangle2dG screenT;
	...
	if (style == wireframe)
	{
		...
		for (int i = 0; i < n; i++)
		{
			...
			drawLine(startP, endP, colour, screen);
		}
	}
	else
	{
		for (int i = 0; i < n - 2; i++)
		{
			...
			{
				this->fillTriangleT(originalT, screenT, screen, true);
			}
			break;
			case sunlight:
			{
				...
				{
					this->fillTriangleT(originalT, screenT, screen, false);
				}
				else
				{
					this->fillTriangleD(originalT, screenT, screen, d);
			...
			case torchlight:
			{
				if (currentTexture.ID > 0)
				{
					this->fillTriangleTTorch(originalT, screenT, screen, d, torchIntensity);
				}
				else
				{
					this->fillTriangleD(originalT, screenT, screen, d);
				...
				if ((i >= 0 && i < w) && (hg >= 0 && hg < h))
				{
					...
					if (invertCurrentZ < screen.depthBuffer[hg * w + i])
					{
						...
						else
						{
							...
							screen.pixelBuffer[hg * w + i] = modifyColour(finalPixel, illCurrent, false, true, false);
						}
...
void Camera::fillTriangleTTorch(triangle3dV T, triangle2dG t, Canvas screen, bool d, double lightIntensity)
{
	coord2 pt[3] = { t.a, t.b, t.c };
	int yMin, yMax;
...
void Camera::drawTriangle(triangle2dG T, Canvas screen)
{
	drawLine(T.a, T.b, T.h, screen);
	drawLine(T.b, T.c, T.h, screen);
	drawLine(T.c, T.a, T.h, screen);
}


void Camera::drawGradientTriangle(triangle2dG T, Canvas screen)
{
	drawGradientLine(T.a, T.b, T.h, screen);
	drawGradientLine(T.b, T.c, T.h, screen);
	drawGradientLine(T.c, T.a, T.h, screen);
}


void Camera::drawLine(coord2 startP, coord2 endP, Uint32 colour, Canvas screen)
{
	...
	if (!diffX && !diffY)
	{
		...
		{
			screen.pixelBuffer[yCurrent * SCREEN_WIDTH + xCurrent] = colour;
		}
	}
	if (!diffX && diffY)
	{
...
void Camera::drawGradientLine(coord2 startP, coord2 endP, Uint32 colour, Canvas screen)
{
	/*No clipping takes place in this method, if either startP or endP
	are off screen, line is discarded entirely. It is assumed that all
	lines form sides of triangles already clipped to the view frustum*/
	...
	if (!diffX && !diffY)
	{
		...
		int xCurrent = startP.x;
		int yCurrent = startP.y;
		double zCurrent = startP.z;
		if ((xCurrent >= 0 && xCurrent < SCREEN_WIDTH) && (yCurrent >= 0 && yCurrent < SCREEN_HEIGHT))
		{
			if (1 / zCurrent < screen.depthBuffer[yCurrent * SCREEN_WIDTH + xCurrent])
			{
				screen.pixelBuffer[yCurrent * SCREEN_WIDTH + xCurrent] = modifyColour(colour, zCurrent, true, true, true);
				screen.depthBuffer[yCurrent * SCREEN_WIDTH + xCurrent] = 1 / zCurrent;
				//if (zCurrent > 1.00) { printf("zCurrent = %.4f\n", zCurrent); }
			}
		}
	}
	if (!diffX && diffY)
	{
		...
		if (startP.y < endP.y)
		{
			x1 = startP.x;	y1 = startP.y;	z1 = startP.z;
			x2 = endP.x;	y2 = endP.y;	z2 = endP.z;
			if ((endP.y - startP.y) != 0)
			{
				stepZ = (startP.z < endP.z) ? (endP.z - startP.z) / ((double)endP.y - (double)startP.y) : (startP.z - endP.z) / ((double)endP.y - (double)startP.y);
			}
			else
			{
				stepZ = 0.0;
		...
		{
			x1 = endP.x;	y1 = endP.y;	z1 = endP.z;
			x2 = startP.x;	y2 = startP.y;	z2 = startP.z;
			if ((endP.y - startP.y) != 0)
			{
				stepZ = (startP.z < endP.z) ? (startP.z - endP.z) / ((double)endP.y - (double)startP.y) : (endP.z - startP.z) / ((double)endP.y - (double)startP.y);
			}
			else
			{
				stepZ = 0.0;
	...
	if (!diffY && diffX)
	{
		...
		if (startP.x < endP.x)
		{
			...
			{
				stepZ = (startP.z < endP.z) ? (endP.z - startP.z) / ((double)endP.x - (double)startP.x) : (startP.z - endP.z) / ((double)endP.x - (double)startP.x);
			}
			else
			{
				stepZ = 0.0;
		...
		else
		{
			...
			if ((endP.x - startP.x) != 0)
			{
				stepZ = (startP.z < endP.z) ? (startP.z - endP.z) / ((double)endP.x - (double)startP.x) : (endP.z - startP.z) / ((double)endP.x - (double)startP.x);
			}
			else
			{
				stepZ = 0.0;
