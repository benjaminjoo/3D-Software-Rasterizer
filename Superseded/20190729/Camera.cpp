...
{
	textCoord testC;
	double deltaX, deltaY, deltaZ, length, deltaXt, deltaYt, deltaZt, t, deltaU, deltaV, d;
	...
	if (length != 0.0) { /*d = sqrt(t / length);*/ d = quickSqrt(t / length); }
	else { d = 0.0; }
...
void Camera::fillTriangleT(triangle3dV T, triangle2dG t, Canvas screen, bool d)
{
	...
	Uint32 finalPixel = 60000;
	txt textureData = currentTexture;
	...
	for (int hg = yMin; hg < yMax; hg++)
	{
		...
		if (endIndex == 2)
		{
			...
			//startVert = this->screen2view(startP, h_ratio, v_ratio);
			invertStartPz = 1 / startP.z;
			startVert.x = (startP.x - halfW) * invertStartPz / hCorr;
			startVert.y = (halfH - startP.y) * invertStartPz / vCorr;
			startVert.z = invertStartPz;
			//startVert.w = 1.0;

			//endVert = this->screen2view(endP, h_ratio, v_ratio);
			invertEndPz = 1 / endP.z;
			endVert.x = (endP.x - halfW) * invertEndPz / hCorr;
			endVert.y = (halfH - endP.y) * invertEndPz / vCorr;
			...
			//To avoid cost of function call
			double _deltaX, _deltaY, _deltaZ, _length, _deltaXt, _deltaYt, _deltaZt, _t, _deltaU, _deltaV, _d;
			_deltaX = sideL[1].x - sideL[0].x;
			_deltaY = sideL[1].y - sideL[0].y;
			_deltaZ = sideL[1].z - sideL[0].z;
			_length = _deltaX * _deltaX + _deltaY * _deltaY + _deltaZ * _deltaZ;
			_deltaXt = startVert.x - sideL[0].x;
			_deltaYt = startVert.y - sideL[0].y;
			_deltaZt = startVert.z - sideL[0].z;
			_t = _deltaXt * _deltaXt + _deltaYt * _deltaYt + _deltaZt * _deltaZt;
			if (_length != 0.0)
			{
				//_d = quickSqrt(_t / _length);

				number = (float)(_t / _length);
				//number = (float)(_length / _t);
				x2 = number * 0.5F;
				y_ = number;
				ii = *(long*)& y_;
				ii = 0x5f3759df - (ii >> 1);
				y_ = *(float*)& ii;
				y_ = y_ * (threehalfs - (x2 * y_ * y_));
				_d =  1 / y_;
				//_d = y_;

			}
			else { _d = 0.0; }
			_deltaU = (uvSideL[1].u - uvSideL[0].u);
			_deltaV = (uvSideL[1].v - uvSideL[0].v);
			startUV.u = uvSideL[0].u + _d * _deltaU;
			startUV.v = uvSideL[0].v + _d * _deltaV;
			//End of inline function
			
			//endUV = getUVCoord(sideR[0], sideR[1], uvSideR[0], uvSideR[1], endVert);
			//To avoid cost of function call
			_deltaX = sideR[1].x - sideR[0].x;
			_deltaY = sideR[1].y - sideR[0].y;
			_deltaZ = sideR[1].z - sideR[0].z;
			_length = _deltaX * _deltaX + _deltaY * _deltaY + _deltaZ * _deltaZ;
			_deltaXt = endVert.x - sideR[0].x;
			_deltaYt = endVert.y - sideR[0].y;
			_deltaZt = endVert.z - sideR[0].z;
			_t = _deltaXt * _deltaXt + _deltaYt * _deltaYt + _deltaZt * _deltaZt;
			if (_length != 0.0)
			{
				//_d = quickSqrt(_t / _length);
			...
			_deltaU = (uvSideR[1].u - uvSideR[0].u);
			_deltaV = (uvSideR[1].v - uvSideR[0].v);
			endUV.u = uvSideR[0].u + _d * _deltaU;
			endUV.v = uvSideR[0].v + _d * _deltaV;
			//End of inline function
					...
					if (1 / zCurrent < screen.depthBuffer[hg * w + i])
					{
						...
						{		
							if (textureData.ID > 0)
							{
								...
								//currentVert = this->screen2view(currentP, h_ratio, v_ratio);
								invertCurrentPz = 1 / currentP.z;
								currentVert.x = (currentP.x - halfW) * invertCurrentPz / hCorr;
								currentVert.y = (halfH - currentP.y) * invertCurrentPz / vCorr;
								...
								//To avoid cost of function call
								_deltaX = endVert.x - startVert.x;
								_deltaY = endVert.y - startVert.y;
								_deltaZ = endVert.z - startVert.z;
								_length = _deltaX * _deltaX + _deltaY * _deltaY + _deltaZ * _deltaZ;
								_deltaXt = currentVert.x - startVert.x;
								_deltaYt = currentVert.y - startVert.y;
								_deltaZt = currentVert.z - startVert.z;
								_t = _deltaXt * _deltaXt + _deltaYt * _deltaYt + _deltaZt * _deltaZt;
								if (_length != 0.0)
								{
									//_d = quickSqrt(_t / _length);
								...
								_deltaU = (endUV.u - startUV.u);
								_deltaV = (endUV.v - startUV.v);
								sampleUV.u = startUV.u + _d * _deltaU;
								sampleUV.v = startUV.v + _d * _deltaV;
								//End of inline function

								sampleXnew = ((int)(double(textureData.w) * sampleUV.u)) % textureData.w;
								sampleYnew = ((int)(double(textureData.h) * sampleUV.v)) % textureData.h;
								...
								{
									if ((sampleXnew >= 0 && sampleXnew < textureData.w) && (sampleYnew >= 0 && sampleYnew < textureData.h))
									{
										finalPixel = textureData.pixels[sampleYnew * textureData.w + sampleXnew];
									}
								}
								...
								screen.pixelBuffer[hg * SCREEN_WIDTH + i] = modifyColour(finalPixel, illCurrent);
