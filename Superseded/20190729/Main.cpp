...
void renderer()
{
	...
	Camera player_01( 0.0, 0.0, 40.0, 1.0, 0.05, -PI * 0.5, 0.0, 0.0, PI * 0.5, 1.0, 999.0, SCREEN_WIDTH, SCREEN_HEIGHT, 0 );
	//Camera player_02( 0.0, 0.0, 90.0, 1.0, 0.1, -PI * 0.5, 0.0, 0.0, PI * 0.75, 1.0, 999.0, SCREEN_WIDTH, SCREEN_HEIGHT, 0 );
	...
	double moveP, strafeP, riseP, stepP, turnP, tiltP;
	moveP = 0;
	strafeP = 0;
	riseP = 0;
	tiltP = 0;
		...
		while (SDL_PollEvent(&event))
		{
			if (event.type == SDL_KEYDOWN)
			{
				...
				case SDLK_m:
					...
					break;
				case SDLK_KP_5:
					World.mouseLookBlocked = World.mouseLookBlocked ? false : true;
					//Player.centreLook();
					//turnV = -PI;
					break;
				case SDLK_p:
			...
			if (event.type == SDL_MOUSEMOTION && !World.mouseLookBlocked)
			{
		...
		{	
			...
			for (int k = 0; k < totalPoly; k++)
			{
				if (Player.polyFacingCamera(meshTriangles[i][k]))			//Backface culling is performed here
				{
					...
					Player.projectPoly(nVert, vertexList, uvList, colour, Screen, hRatio, vRatio,
