...
void SolidRevolved::getTriangleData_(triangle3dV* t)
{
	...
	int nTriangle = this->getTotalPoly();
	int nNodes = this->numberOfNodes();
	triangle3dV* tt = this->getTriangleDataFlat();
		...
		for (int i = 0; i < resolH; i++)
		{
			...
			if (i < resolH - 1)
			{
				if (cT < nTriangle)
				{
					...
					t[cT].At.u = 0.0;				t[cT].At.v = 0.0;
					t[cT].Bt.u = 0.0;				t[cT].Bt.v = 0.0;
					t[cT].Ct.u = 0.0;				t[cT].Ct.v = 0.0;
...
void SolidRevolved::constructShadowVolume(vect3)
{
