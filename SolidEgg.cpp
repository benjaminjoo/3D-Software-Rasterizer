...
SolidEgg::SolidEgg(double px, double py, double pz, Uint32 c)
{
	scale		= { 1.0, 1.0, 1.0, 1.0 };
...
SolidEgg::SolidEgg(double sx, double sy, double sz, double px, double py, double pz, double rx, double ry, double rz, double dist,
...
int	SolidEgg::getTotalVert()
{
	return 2 * resol* (resol - 1) + 2;
}


int	SolidEgg::getTotalPoly()
{
	return 4 * resol* (resol - 1);
}


void SolidEgg::getVertexData_(vect3* s)
{
			...
			for (int j = 0; j < resol * 2; j++)
			{
				...
				if (cnt < total)
				{
					if (cnt < total)
					{
						...
						s[cnt].z = (i > int(resol / 2)) ? (radius * sin(angleV) * distortion) : (radius * sin(angleV));
...
void SolidEgg::getTriangleData_(triangle3dV* t)
{
...
vect3 SolidEgg::getPosition()
{
	return position;
}


void SolidEgg::constructShadowVolume(vect3)
{
